...
void __fastcall ThreadSerialBufferIn::PlotCH1Signal() {
	frmPrincipal->lineSeriesCH1Signal->AddXY(FCount, FCH1SignalValue, "",
		clTeeColor);
...
void __fastcall ThreadSerialBufferIn::PlotCH2Signal() {
	frmPrincipal->lineSeriesCH1Signal->AddXY(FCount, FCH2SignalValue, "",
		clTeeColor);
	if (FCount > PLOT_WINDOW) {
		FCount = 0;
		frmPrincipal->ClearChartSeries();
...
void __fastcall ThreadSerialBufferIn::Execute() {
	...
	while (1) {
		...
		if ((FSerialBuffer.at(0) == '#') && (FSerialBuffer.at(1) == '$') &&
			(FSerialBuffer.at(2) == ':')) {
			...
			if (FPackageList.size()) {
				for (int index = 8; index <= FPackageList.size(); index += 8) {
					...
					FCH1SignalValue =
						((unsigned char)(ASignal.at(3)) << 8 | (unsigned char)
						(ASignal.at(4))); // * ADC_1024;
					FCH2SignalValue =
						((unsigned char)(ASignal.at(5)) << 8 | (unsigned char)
						(ASignal.at(6))); // * ADC_1024;
...
void __fastcall ThreadSerialBufferIn::ClearSerialBuffer() {
	while ((FSerialBuffer.size() >= 3) && ((FSerialBuffer.at(0) != '#') ||
		(FSerialBuffer.at(1) != '$') || (FSerialBuffer.at(2) != ':')))
